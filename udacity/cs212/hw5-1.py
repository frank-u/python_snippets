...
def Q_pig(me, you, pending, action, Pwin):
    "The expected value of choosing action in state."
    if action == 'hold':
        return 1 - Pwin3(you, me + pending, 0)
    if action == 'roll':
        return (1 - Pwin3(you, me + 1, 0) + sum(
            Pwin3(me, you, pending + d) for d in (2, 3, 4, 5, 6))) / 6.
    raise ValueError


def pig_actions(pending):   return ['roll', 'hold'] if pending else ['roll']
...
def Pwin3(me, you, pending):
    ...
    if me + pending >= goal:
        ...
        return max(Q_pig(me, you, pending, action, Pwin3) for action in
                   pig_actions(pending))
...
# print Pwin2((0, 42, 25, 0))
#print Pwin2((1, 12, 43, 0))